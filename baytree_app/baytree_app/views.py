...
def create_object(object_dict, model):
    ...
    for key, val in object.items():
        ...
        if field.many_to_many != True \
            ...
            create_args[key] = val
        elif field.one_to_one == True \
                or isinstance(field, ForeignKey) == True:
    ...
    for key, val in object.items():  # many to many fields
        if model._meta.get_field(key).many_to_many == True:
            app_and_model = model._meta.get_field(
                key).target_field.cached_col.identity[1][1].split("_")
...
class CookieTokenVerifySerializer(TokenVerifySerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs['token']:
            ...
            raise InvalidToken(
                'No valid token found in cookie')
...
class CookieTokenVerifyView(TokenVerifyView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"is_admin": is_admin, "is_superuser": is_superuser}, status=status.HTTP_200_OK)
...
class CookieTokenObtainPairView(TokenObtainPairView):
    def finalize_response(self, request, response, *args, **kwargs):
        if response.data.get('refresh'):
            ...
            response.set_cookie(
                'refresh_token', response.data['refresh'], max_age=cookie_max_age, httponly=True, samesite="Strict")
            del response.data['refresh']
        if response.data.get('access'):
            cookie_max_age = 60 * 30  # 30 min
            response.set_cookie(
                'access_token', response.data['access'], max_age=cookie_max_age, httponly=True, samesite="Strict")
...
class CookieTokenRefreshView(TokenRefreshView):
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        if access_token_obj != None:
            ...
            admin_user_model = apps.get_model("users", "AdminUser")
            response.data['is_admin'] = admin_user_model.objects.filter(pk=user_id).exists()
    ...
    def get(self, request):
        ...
        if ('id' not in request.GET):
            ...
            serialized = [self.serializer(
                modelObject).data for modelObject in modelObjects]

            return Response({"total": numModelObjects, "data": serialized}, status=status.HTTP_200_OK)
        else:
            ...
            return Response({"total": numModelObjects, "data": objects}, status=status.HTTP_200_OK)
    ...
    def delete(self, request):
        ...
        if (ids != None):
            try:
                ...
                return Response({"status": "Successfully deleted object(s)."}, status=status.HTTP_200_OK)
            except Exception as e:
                return Response({"error": "Failed to delete object"},
                                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        else:
            return Response({"error": "No id was given"},
    ...
    def put(self, request):
        ...
        try:
            if "array" in request.data:
                ...
                return Response({"success": "Successfully updated objects"},
                                status=status.HTTP_200_OK)
            else:
                object = request.data
