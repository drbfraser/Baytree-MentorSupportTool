...
ln -s -f ~/baytree/scripts/update.sh ~/update.sh
...
echo -e "   1: Seed one admin user (recommended)"
...
case $OPTION in
    1)
        ...
        echo -e "\n${BLUE}Please enter the email address for the new admin: ${COLOR_OFF}"
        read;

        docker exec baytree_server python ./manage.py seedadminuser ${REPLY}
        
        echo -e "\n${RED}** WRITE DOWN AND SAVE THE PASSWORD ABOVE! **${COLOR_OFF}"
        ;;
esac
