...
const RecordDetail: FunctionComponent<Props> = ({
  sessionId,
  handleClose,
  ...props
}) => {
    ...
    return (
      <>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between' }}>
          {session.name}
          <Chip
            label={isAttended ? 'ATTENDED' : 'CANCELLED'}
            color={isAttended ? 'success' : 'error'}
          />
        </DialogTitle>
        <DialogContent>
          ...
          <Grid container sx={{ mt: 2 }} spacing={2}>
            <Grid item xs={12} sm={12} md={4}>
              ...
              <InfoTextField
                value={formatInTimeZone(startTime, TIMEZONE_ID, 'do MMMM Y')}
              />
            </Grid>
            <Grid item xs={12} sm={12} md={4}>
              <Typography sx={{ mb: 1 }}>
              ...
              <InfoTextField
                value={formatInTimeZone(startTime, TIMEZONE_ID, 'hh:mm aa')}
              />
            </Grid>
            <Grid item xs={12} sm={12} md={4}>
              <Typography sx={{ mb: 1 }}>
