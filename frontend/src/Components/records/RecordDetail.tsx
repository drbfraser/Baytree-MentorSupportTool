...
const RecordDetailContent: FunctionComponent<{ sessionId: string | number } & Props> = ({ sessionId, handleClose }) => {
  const { session, isLoading, sessionError, isSubmitting, note, setNote, handleUpdateNote, isFetching } = useSessionDetail(sessionId);
  ...
  return <>
    ...
    <DialogActions>
      ...
      <LoadingButton
        ...
        disabled={isFetching || !note || note === (session.note || "")}
        variant="contained"
...
const RecordDetail: FunctionComponent<{ sessionId?: string | number } & Props> = ({ sessionId, handleClose, ...props }) => {
  return <Dialog {...props} fullWidth maxWidth="md">
    {!sessionId ? <Alert severity="error">
      <AlertTitle>No session ID provided</AlertTitle>
      Please refresh the page or contact the adminstrator
    </Alert> : <RecordDetailContent sessionId={sessionId} handleClose={handleClose} {...props} />}
  </Dialog>
}
