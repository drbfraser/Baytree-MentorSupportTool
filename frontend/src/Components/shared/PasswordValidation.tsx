  ...
  return (
    <Box display="flex" alignItems="center">
      <Icon
        ...
        sx={{ m: 0.5 }}
      />
      <Typography variant="subtitle2" color={validated ? 'primary' : 'error'}>
        {label}
      </Typography>
    </Box>
  )
...
export const isValid = (password: string, passwordAgain: string) =>
  [
    ...
    hasUppercase,
    hasSpecial,
    isMatch(passwordAgain)
  ]
    .map((check) => check(password))
    .every((result) => result)

const PasswordValidation: FunctionComponent<{
  password: string
  ...
  return (
    <Box padding={1} sx={{ backgroundColor: 'rgba(0, 0, 0, 0.05)', my: 2 }}>
      <ValidationMessage
        validated={hasValidLength(password)}
        label="Contains 8 - 30 characters"
      />
      <ValidationMessage
        validated={hasNumber(password)}
        label="Contains at least one number"
      />
      <ValidationMessage
        validated={hasLowercase(password)}
        label="Contains at least one lowercase letter"
      />
      <ValidationMessage
        validated={hasUppercase(password)}
        label="Contains at least one uppercase letter"
      />
      <ValidationMessage
        validated={hasSpecial(password)}
        label="Contains at least one special character"
      />
      <ValidationMessage
        validated={isMatch(password)(passwordAgain)}
