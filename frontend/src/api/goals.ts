...
export const fetchGoals = async (
  ...
  if (query.categoryIds && query.categoryIds.length > 0)
    params.categories = query.categoryIds.join(',')
  if (query.orderingDate)
    params.ordering = (!query.ascending ? '-' : '') + query.orderingDate
...
export const submitGoal = async (input: GoalInput, id?: number) => {
  ...
  try {
    const promise = id
      ? goalsApi.put<Goal>(`${id}/`, data)
      : goalsApi.post<Goal>('', data)
