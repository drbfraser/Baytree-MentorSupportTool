...
export const getMenteeIdsForMentor = async () => {
  try {
    const apiRes = await viewsApi.get<number[]>("/mentor-mentee-ids");
    ...
    return {
      data: undefined,
      error: "Cannot retrieve mentee ids for current logged in mentor."
    };
  }
};
